...
export function useSocket(roomId: string | null) {
  ...
  useEffect(() => {
    ...
    const handleSyncState = (state: RoomState) => {
      ...
      if (!store.isHost) {
        const driftMs = Math.abs(
          audioManager.getCurrentTimeMs() - state.current_time_ms,
        );
        if (driftMs > 700) {
    ...
    const handleRoomParticipants = (data: RoomParticipantsPayload) => {
      const participants = data.participants.map((p) => ({
        id: p.sid,
        name: p.name,
        role: p.role === "host" ? ("host" as const) : ("listener" as const),
        isOnline: true,
      }));
      useAppStore.setState({
        participants,
        isHost: participants.some(
          (p) => p.role === "host" && p.id === socketManager.socketId,
        ),
      });
    };
    ...
    socketManager.on("room_participants", cast(handleRoomParticipants));
    ...
    return () => {
      ...
      socketManager.off("room_participants", cast(handleRoomParticipants));
